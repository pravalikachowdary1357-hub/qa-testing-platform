...
interface DeleteTestScenarioDialogProps {
  testScenario: ApiTestScenario | null;
  onClose: () => void;
...
export function DeleteTestScenarioDialog({
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to delete test scenario.');
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <Dialog open={Boolean(testScenario)} onClose={onClose} fullWidth maxWidth="xs">
      ...
      <DialogContent>
        ...
        <DialogContentText>
          Are you sure you want to delete <strong>{testScenario?.title}</strong>? This action
          cannot be undone.
        </DialogContentText>
      </DialogContent>
