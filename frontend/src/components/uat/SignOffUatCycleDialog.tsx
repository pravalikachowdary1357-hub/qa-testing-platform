...
import type { SignOffUatCyclePayload, UatCycleListItem } from '../../types/uat';

interface SignOffUatCycleDialogProps {
  cycle: UatCycleListItem | null;
  onClose: () => void;
  onConfirm: (data: SignOffUatCyclePayload) => Promise<void>;
}

export function SignOffUatCycleDialog({ cycle, onClose, onConfirm }: SignOffUatCycleDialogProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      await onConfirm({ decision, signOffBy: trimmedSignOffBy, notes: notes.trim() || undefined });
      onClose();
    } catch (err: unknown) {
  ...
  return (
    <Dialog open={Boolean(cycle)} onClose={onClose} fullWidth maxWidth="xs">
      <DialogTitle>Sign Off UAT Cycle</DialogTitle>
      <DialogContent>
        {error && (
        ...
        <DialogContentText sx={{ mb: 2 }}>
          Record the final approval decision for <strong>{cycle?.name}</strong>.
        </DialogContentText>
        <Stack spacing={2}>
          <TextField
            select
